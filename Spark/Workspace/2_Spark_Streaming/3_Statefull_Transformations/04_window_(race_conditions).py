...
def my_model(ssc, monitoring_dir, result_dir, time_step_interval, window_duration, sliding_duration, bad_chars):
    ...
    # 2. Operation T1: Transformation window, which puts together different micro-batches.
    ...
    # 3. Operation T2: Transformation 'flatMap', so as to get a new DStream where each underlying RDD contains all the words of its equivalent
    # RDD in windowDStream.
    allWordsDStream = windowDStream.flatMap(lambda x: process_line(x, bad_chars))

    # 4. Operation T3: Transformation 'map', so as to get a new DStream where each underlying RDD contains pair items, versus the single String items of
    # its equivalent RDD in allWordsDStream.
    pairWordsDStream = allWordsDStream.map(lambda x: (x, 1))

    # 5. Operation T4: Transformation 'reduceByKey', so as to get a new DStream where each underlying RDD aggregates the amount of times each word
...
def my_main(sc,
    ...
    # Please note that the case with 0 and 1 is equivalent to any other example with k and k+1 where k is an odd number.
...
if __name__ == '__main__':
    ...
    race_conditions_extra_delay = False
