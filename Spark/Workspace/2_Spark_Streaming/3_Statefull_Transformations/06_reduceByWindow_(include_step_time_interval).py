...
# ------------------------------------------
# FUNCTION num_valid_words_in_line
# ------------------------------------------
def num_valid_words_in_line(line, bad_chars):
    ...
    # 7. Instead of returning the list
    res = len(valid_words)
...
def my_model(ssc, monitoring_dir, result_dir, time_step_interval, window_duration, sliding_duration, bad_chars):
    ...
    # 2. Operation T1: Transformation map, so as to get a new DStream where each underlying RDD contains all the words per line of its equivalent
    # RDD in inputDStream.
    wordsPerLineDStream = inputDStream.map(lambda x: num_valid_words_in_line(x, bad_chars))

    # 3. Opetion T2: Transformation reduceByWindow, so as to get the reduction on the window more efficiently.
    # The first argument the reduce function F1 to be used over the whole window, such as +.
    # Optionally they accept as a second argument a function F2 acting as the inverse of F1.
    # F2 is used for efficiency reasons, as it allows to compute the reduction incrementally,
    # by considering only which data is coming into the window and which data is going out.
    ...
    # Super interesting topic: TIME STEP INTERVAL big enough!
    # In the code example s04 we only considered two time variables, X and Y.
    # X represents the time in which the StreamContext ssc will check monitoring_dir for first time.
    # Y is the time in which the first file is transferred from source_dir to monitoring_dir.

    # Perfect, this is absolutely truth, but both X and Y are impacted by time_step_interval, set by the user,
    # and which determines how often ssc checks and files are transferred.
    # Yes, but not just this. In fairness, time_step_interval represents how much does a step last in our Spark program executions,
    # on, in other words, how often do things happen.

    # It is responsability of the user to set this to a big enough amount of time. Otherwise, people are rush up, they do not have
    # time to complete their job, and end up producing results that are senseless to us: Windows that do not correspond with what
    # they should, results that are presented incomplete, etc.

    # In this tiny example above, commands #3 is run every 2 steps (requesting the results of the last 3 steps).
    # If a result is not completed in time, #3 doesn't stop and wait, but takes whatever it is there for the step
    # (if there is absolutely nothing, it is just understood as an empty RDD for that concrete step).

    # Thus, once again, be careful with the window operations, and set the time step to a big enough value, such that the window
    # is not filled with wrong information (missing or incomplete results).

    # In the above example, try with steps of 1 second, 3 seconds and 10 seconds, and see the different results.

    # 4. Operation P1: We persist solutionDStream, as we are going to use it in more than 1 output operation.
    solutionDStream.cache()

    # 5. Operation OO1: Output Operation pprint() so that we print the content of solutionDStream
    solutionDStream.pprint()

    # 6. Operation OO2: Output Operation saveAsTextFiles so as to Store the DStream solutionDStream into the desired folder from the DBFS.
