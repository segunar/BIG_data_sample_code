...
def my_model(ssc, monitoring_dir, result_dir):
    ...
    # 2. We sort each micro-batch (RDD of String items) by the amount of words each line (item) contains.

    # Option 1: Use the Transformation Operation sortBy we used in 07_sortBy.py
    # However, as we can see, this Transformation is not a built-in DStream function.
    # When we try to use it directly we get the following error:
    # AttributeError: 'DStream' object has no attribute 'sortBy'

    # solutionDStream = inputDStream.sortBy(lambda line: len(line.split(" "))) --> Leads to an error

    # Personally, I don't understand why DStream does not directly include all transformations as built-in functions.
    # In any case, for these transformations not included, and for any new transformation we want to define ourselves,
    # Spark Streaming provide the advanced operator transform(), which lets you operate directly on the RDDs inside them.
    # The transform() operation lets you provide any arbitrary RDD-to-RDD function to act on the DStream.
    # This function gets called on each batch of data in the DStream to produce a new DStream.

    solutionDStream = inputDStream.transform(
        lambda rdd: rdd.flatMap(lambda item: item.split(" ")).sortBy(lambda item: len(item))
                                            )
